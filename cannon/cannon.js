...
Q.scene('level', new Q.Scene(function(stage){
    ...
    stage.insert(new Q.Sprite({w: 70, h: 10, x: 525, y: 100}));
    ...
    stage.insert(new Q.Sprite({w: 30, h: 30, x: 10, y: 210, color: 'blue'}));
...
var cannonMove = function(e){
    var stage = Q.stage(0),
        cannon = stage.cannon,
        touch = e.changeTouches
            ? e.changedTouches[0]
            : e,
        point = stage.browserToWorld(touch.pageX, touch.pageY);

    var angle = Math.atan2(point.y - cannon.p.y, point.x - cannon.p.x);
...
var cannonFire = function(e){
    Q.stage(0).cannon.fire();
    e.preventDefault();
};

Q._each(['touchend', 'mouseup'], function(evt){
    Q.wrapper.addEventListener(evt, cannonFire);
});
